...
import { useFrame, useThree } from '@react-three/fiber'; // Added useThree import
import { a } from '@react-spring/three';

import islandScene from '../assets/3d/island.glb'; // Corrected the path

const Island = ({ isRotating, setIsRotating,setCurrentStage, ...props }) => {
  const islandRef = useRef();

  const { gl, viewport } = useThree(); // Destructure useThree for gl and viewport
  const { nodes, materials } = useGLTF(islandScene); // Use the correct import
  ...
  const handlePointerDown = (e) => {
    ...
    const clientX = e.touches ? e.touches[0].clientX : e.clientX; // Use 'e' instead of 'event'
  ...
  const handlePointerMove = (e) => {
    ...
    if (isRotating) {
      ...
      islandRef.current.rotation.y += delta * 0.05 * Math.PI; // Slower
